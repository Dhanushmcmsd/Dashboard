import { getBranchPerformance } from "@/lib/gold-loan/data-access";
...
export async function GET(req: Request) {
  try {
    ...
    const data = await getBranchPerformance(companySlug, periodType, asOnDate);
    return successResponse(data);
  } catch (error) {
    const message = error instanceof Error ? error.message : "Failed to fetch branch performance.";
