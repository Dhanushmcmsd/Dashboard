...
async function base(companyId: string, portfolioType: string, period: string) {
  return pool.query(
    `SELECT ks.* FROM kpi_snapshots ks
     ...
     ORDER BY ks.as_of_date DESC LIMIT 1`,
    [companyId, portfolioType, period]
  );
}
...
r.get("/:companyId/:portfolioType", requireAuth, requireCompanyScope, validate(paramsSchema, "params"), validate(qSchema, "query"), async (req, res) => {
  const row = await base(req.params.companyId, req.params.portfolioType, (req.query as any).period);
  res.json(row.rows[0] || null);
});
r.get("/:companyId/:portfolioType/kpis", requireAuth, requireCompanyScope, validate(paramsSchema, "params"), validate(qSchema, "query"), async (req, res) => res.json((await base(req.params.companyId, req.params.portfolioType, (req.query as any).period)).rows[0] || null));
