...
export async function computeKpisForUpload(uploadId: string): Promise<void> {
  const { rows } = await pool.query(
    `SELECT id, portfolio_id, statement_date FROM data_uploads WHERE id = $1`,
    [uploadId]
  );
  if (!rows[0]) return;
  ...
  for (const p of periods) {
    await pool.query(
      ...
      DO UPDATE SET
        ...
        computed_at=now()`,
      [portfolio_id, p.s.toISOString().slice(0, 10), asOf.toISOString().slice(0, 10), p.t]
    );
  }
}
