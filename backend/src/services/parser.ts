...
const txnAliases: Record<string, string[]> = {
...
const toPaise = (v: unknown) => (v == null || v === "" ? null : Math.round(Number(v) * 100));
const toMg = (v: unknown) => (v == null || v === "" ? null : Math.round(Number(v) * 1000));
...
export function parseLoanBalance(buffer: Buffer) {
  ...
  if (hasMissing) throw new Error("Missing required closing_balance or dpd_days");
  return out;
}

export function parseTransactions(buffer: Buffer) {
  const wb = XLSX.read(buffer, { type: "buffer" });
  const ws = wb.Sheets[wb.SheetNames[0]];
  const rows: any[][] = XLSX.utils.sheet_to_json(ws, { header: 1, defval: "" });
  const headers = (rows[0] || []).map(String);
