...
async function handle(req, res, statementType) {
    ...
    if (uploaded.error)
        return res.status(500).json({ error: uploaded.error.message });
    const q = await pool.query(`INSERT INTO data_uploads (portfolio_id, uploaded_by, statement_type, file_url, statement_date, row_count, status)
     VALUES ($1,$2,$3,$4,$5,0,'pending') RETURNING id`, [portfolioId, req.user.id, statementType, key, statementDate]);
...
r.get("/", requireAuth, async (req, res) => {
    const rows = await pool.query(`SELECT du.* FROM data_uploads du JOIN portfolios p ON p.id=du.portfolio_id WHERE p.company_id=$1 ORDER BY du.created_at DESC`, [req.user?.company_id]);
    res.json(rows.rows);
});
r.get("/:id/status", requireAuth, async (req, res) => {
    const row = await pool.query(`SELECT du.id, du.status, du.error_log FROM data_uploads du JOIN portfolios p ON p.id=du.portfolio_id WHERE du.id=$1 AND p.company_id=$2`, [req.params.id, req.user?.company_id]);
    if (!row.rows[0])
